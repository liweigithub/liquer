package contentpagelib 
{
    ...
    {
        public function createtext(arg1:flash.display.Stage, arg2:flash.display.MovieClip)
        {
            ...
            this.solidbg.graphics.beginFill(this._root.contentbgcolor);
            if (this._root.contentheight == 0) 
            {
                this.solidbg.graphics.drawRect(0, 0, this._root.bannerwidth, 10);
            }
            else if (this._root.contentbgrounded != 0) 
            {
                this.solidbg.graphics.drawRoundRect(0, 0, this._root.bannerwidth, this._root.contentheight, this._root.contentbgrounded, this._root.contentbgrounded);
            }
            else 
            {
                this.solidbg.graphics.drawRect(0, 0, this._root.bannerwidth, this._root.contentheight);
            }
            this.solidbg.graphics.endFill();
            this.solidbg.alpha = this._root.contentbgopacity;
            this.txtmask.graphics.beginFill(this._root.contentbgcolor);
            if (this._root.contentheight == 0) 
            {
                this.txtmask.graphics.drawRect(0, 0, this._root.bannerwidth, 10);
            }
            else 
            {
                this.txtmask.graphics.drawRect(0, 0, this._root.bannerwidth, this._root.contentheight - this._root.contentbgtextvpadding * 2);
            }
            ...
            this.txt.width = this._root.bannerwidth - this._root.contentbgtexthpadding * 2 - this._root.contentscrollbarwidth - 10;
            if (this._root.textfieldheight != 0) 
            {
                this.txt.height = this._root.textfieldheight;
            }
            this.txt.styleSheet = this._root.style;
            this.txt.htmlText = this._root.contenttext;
            ...
            {
                this.solidbg.y = this._root.bannerheight + this._root.contenttopdistance;
                this.txt_mc.y = this._root.bannerheight + this._root.contenttopdistance + this._root.contentbgtextvpadding;
            }
        ...
        {
            this.solidbg.height = this._root.verticalspace - this._root.bannerheight - this._root.contenttopdistance;
            this.txtmask.height = this._root.verticalspace - this._root.bannerheight - this._root.contenttopdistance - this._root.contentbgtextvpadding * 2;
            this.initScroll();
            return;
        }

        internal function removeScrollListeners():void
        {
