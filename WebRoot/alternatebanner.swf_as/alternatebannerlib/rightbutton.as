package alternatebannerlib 
{
    ...
    public class rightbutton extends flash.display.MovieClip
    {
        public function rightbutton(arg1:flash.display.MovieClip, arg2:flash.display.MovieClip)
        {
            this.btnmc = new flash.display.MovieClip();
            this.btnicon = new banner_right();
            super();
            this._root = arg1;
            this._banner = arg2;
            if (this._root.navigplaypause != "true") 
            {
                this.buttonpos = Math.round(this._root.bannerwidth / 2) + Math.round(this._root.navigbuttonspacing / 2);
            }
            else 
            {
                this.buttonpos = Math.round(this._root.bannerwidth / 2) + 15 + this._root.navigbuttonspacing;
            }
        ...
        {
            if (this._banner.clickallow && this._banner.clickallow2) 
            {
                this._banner.clickallow = false;
                this._banner.clickallow2 = false;
                this._banner.setTimer();
